...
def train(local_rank, config):  # the first arg must be local rank for the sake of using mp.spawn(...)
    ...
    if config.datasets.dataset_name == 'shapenet_Yi650M':
        ...
        train_set, validation_set, trainval_set, test_set = dataloader.get_shapenet_dataset_AnTao350M(config.datasets.url, config.datasets.saved_path, config.datasets.selected_points, config.train.dataloader.data_augmentation.enable, config.train.dataloader.data_augmentation.jitter.enable,
